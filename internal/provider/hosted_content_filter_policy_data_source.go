...
var (
	_ datasource.DataSource              = &hostedContentFilterPolicyDataSource{}
	_ datasource.DataSourceWithConfigure = &hostedContentFilterPolicyDataSource{}
)

type hostedContentFilterPolicyDataSource struct{ client *clients.Client }

// NewHostedContentFilterPolicyDataSource looks up a HostedContentFilterPolicy object (Get-HostedContentFilterPolicy).
func NewHostedContentFilterPolicyDataSource() datasource.DataSource {
	return &hostedContentFilterPolicyDataSource{}
}

func (d *hostedContentFilterPolicyDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_hosted_content_filter_policy"
}

func (d *hostedContentFilterPolicyDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Look up an existing HostedContentFilterPolicy object. Set identity or name to select it.",
...
func (d *hostedContentFilterPolicyDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, _ *datasource.ConfigureResponse) {
...
func (d *hostedContentFilterPolicyDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data hostedContentFilterPolicyModel
	...
	get := func(ctx context.Context) (map[string]any, bool, error) {
		res, gerr := d.client.EXO.GetHostedContentFilterPolicy(ctx, exo.GetHostedContentFilterPolicyParams{Identity: identity})
		if gerr != nil {
			if isNotFound(gerr) {
	...
	if !present {
		resp.Diagnostics.AddError("HostedContentFilterPolicy not found", "no HostedContentFilterPolicy object for identity "+identity)
		return
	}
	readHostedContentFilterPolicy(ctx, obj, &data)
