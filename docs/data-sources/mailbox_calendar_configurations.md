---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "exo_mailbox_calendar_configurations Data Source - exo"
subcategory: ""
description: |-
  List every MailboxCalendarConfiguration object (Get-MailboxCalendarConfiguration).
---

# exo_mailbox_calendar_configurations (Data Source)

List every MailboxCalendarConfiguration object (Get-MailboxCalendarConfiguration).



<!-- schema generated by tfplugindocs -->
## Schema

### Read-Only

- `mailbox_calendar_configurations` (Attributes List) All MailboxCalendarConfiguration objects. (see [below for nested schema](#nestedatt--mailbox_calendar_configurations))

<a id="nestedatt--mailbox_calendar_configurations"></a>
### Nested Schema for `mailbox_calendar_configurations`

Read-Only:

- `agenda_mail_enabled` (Boolean) Maps to the -AgendaMailEnabled parameter.
- `agenda_mail_introduction_enabled` (Boolean) Maps to the -AgendaMailIntroductionEnabled parameter.
- `agenda_pane_enabled` (Boolean) Maps to the -AgendaPaneEnabled parameter.
- `auto_decline_when_busy` (Boolean) Maps to the -AutoDeclineWhenBusy parameter.
- `calendar_feeds_preferred_language` (String) Maps to the -CalendarFeedsPreferredLanguage parameter.
- `calendar_feeds_preferred_region` (String) Maps to the -CalendarFeedsPreferredRegion parameter.
- `calendar_feeds_root_page_id` (String) Maps to the -CalendarFeedsRootPageId parameter.
- `conversational_scheduling_enabled` (Boolean) Maps to the -ConversationalSchedulingEnabled parameter.
- `create_events_from_email_as_private` (Boolean) Maps to the -CreateEventsFromEmailAsPrivate parameter.
- `daily_agenda_mail_schedule` (String) Maps to the -DailyAgendaMailSchedule parameter.
- `default_online_meeting_provider` (String) Maps to the -DefaultOnlineMeetingProvider parameter.
- `default_reminder_time` (String) Maps to the -DefaultReminderTime parameter.
- `delete_meeting_request_on_respond` (Boolean) Maps to the -DeleteMeetingRequestOnRespond parameter.
- `dining_events_from_email_enabled` (Boolean) Maps to the -DiningEventsFromEmailEnabled parameter.
- `entertainment_events_from_email_enabled` (Boolean) Maps to the -EntertainmentEventsFromEmailEnabled parameter.
- `events_from_email_enabled` (Boolean) Maps to the -EventsFromEmailEnabled parameter.
- `first_week_of_year` (String) Maps to the -FirstWeekOfYear parameter.
- `flight_events_from_email_enabled` (Boolean) Maps to the -FlightEventsFromEmailEnabled parameter.
- `hotel_events_from_email_enabled` (Boolean) Maps to the -HotelEventsFromEmailEnabled parameter.
- `id` (String) Object identifier (GUID).
- `identity` (String) Identity used to target the object.
- `invoice_events_from_email_enabled` (Boolean) Maps to the -InvoiceEventsFromEmailEnabled parameter.
- `location_details_in_free_busy` (String) Maps to the -LocationDetailsInFreeBusy parameter.
- `mailbox_location` (String) Maps to the -MailboxLocation parameter.
- `online_meetings_by_default_enabled` (String) Maps to the -OnlineMeetingsByDefaultEnabled parameter.
- `package_delivery_events_from_email_enabled` (Boolean) Maps to the -PackageDeliveryEventsFromEmailEnabled parameter.
- `preserve_declined_meetings` (Boolean) Maps to the -PreserveDeclinedMeetings parameter.
- `reminder_sound_enabled` (Boolean) Maps to the -ReminderSoundEnabled parameter.
- `reminders_enabled` (Boolean) Maps to the -RemindersEnabled parameter.
- `rental_car_events_from_email_enabled` (Boolean) Maps to the -RentalCarEventsFromEmailEnabled parameter.
- `service_appointment_events_from_email_enabled` (Boolean) Maps to the -ServiceAppointmentEventsFromEmailEnabled parameter.
- `shorten_event_scope_default` (String) Maps to the -ShortenEventScopeDefault parameter.
- `show_week_numbers` (Boolean) Maps to the -ShowWeekNumbers parameter.
- `skip_agenda_mail_on_free_days` (Boolean) Maps to the -SkipAgendaMailOnFreeDays parameter.
- `time_increment` (String) Maps to the -TimeIncrement parameter.
- `use_bright_calendar_color_theme_in_owa` (Boolean) Maps to the -UseBrightCalendarColorThemeInOwa parameter.
- `weather_enabled` (String) Maps to the -WeatherEnabled parameter.
- `weather_locations` (String) Maps to the -WeatherLocations parameter.
- `weather_unit` (String) Maps to the -WeatherUnit parameter.
- `week_start_day` (String) Maps to the -WeekStartDay parameter.
- `work_days` (String) Maps to the -WorkDays parameter.
- `working_hours_end_time` (String) Maps to the -WorkingHoursEndTime parameter.
- `working_hours_start_time` (String) Maps to the -WorkingHoursStartTime parameter.
- `working_hours_time_zone` (String) Maps to the -WorkingHoursTimeZone parameter.
- `workspace_user_enabled` (String) Maps to the -WorkspaceUserEnabled parameter.
